...
def test_ekf(**kwargs):
    ...
    for t in range(1, data.num_samples):
        Q[t] = ekf.update(DEG2RAD*data.gyr[t], data.acc[t], data.mag[t], Q[t-1])
        euler_angles[t] = ahrs.common.orientation.q2euler(ahrs.common.orientation.q_conj(Q[t]))*RAD2DEG

    if plot:
        # Plot Signals
        import matplotlib.pyplot as plt
        ahrs.utils.plot_sensors(data.gyr, data.acc, data.mag, x_axis=data.time, title="Sensors: EKF")
        ahrs.utils.plot_euler(euler_angles, x_axis=data.time, title="Euler Angles: EKF")
        ahrs.utils.plot_quaternions(Q, x_axis=data.time, title="Quaternions: EKF")
        plt.show()
    # Test computed data
